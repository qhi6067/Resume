...
// resumeData.js uses `export const` — strip the keyword then eval in a clean scope.
...
// ── LaTeX escaper (user-data only — never applied to template strings) ───────
function esc(str) {
  if (str == null) return '';
  return String(str)
    .replace(/[\\&%$#_{}~^]/g, c => ({
...
function genSkillsAndTools() {
  ...
  if (LANGUAGES?.length) {
    lines.push(`     \\textbf{Languages}{: ${LANGUAGES.map(esc).join(', ')}} \\\\`);
  }

  for (const [key, label] of Object.entries(SKILL_LABELS)) {
    const items = SKILLS[key];
    if (items?.length) {
      lines.push(`     \\textbf{${label}}{: ${items.map(esc).join(', ')}} \\\\`);
    }
  }
...
// Locate pdflatex — check PATH first, then the MiKTeX default install location on Windows
...
if (!hasPdflatex) {
  console.log('');
  console.log('⚠  pdflatex not found — .tex file written but PDF not compiled.');
  console.log('   Install MiKTeX (https://miktex.org) or TeX Live, then re-run:');
  console.log('       npm run generate-resume');
  process.exit(0);
}

// Compile (run twice: first pass builds aux, second pass finalises layout)
